...
public class FlutterACPPlacesPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    private void getNearbyPointsOfInterest(final Result result, final Object arguments) {
        ...
        // ACP Places SDK (1.x) is incompatible with AEP Core (2.x) at runtime.
        // Calling Places.getNearbyPointsOfInterest causes a crash due to SDK conflict.
        // Return empty list — user can add POIs manually via the UI.
        Log.w(LOG_TAG, "getNearbyPointsOfInterest: bypassed due to ACP/AEP SDK incompatibility");
        result.error("PLACES_SDK_UNAVAILABLE",
            "Adobe Places SDK is not available in this build. Please add POIs manually.",
            null);
    }
    ...
    private void processGeofence(final Object arguments) {
        ...
        // radius and expirationDuration may arrive as Integer or Double from Dart
        final Number radius = (Number) geofenceMap.get(RADIUS);
        final Number expirationDuration = (Number) geofenceMap.get(EXPIRATION_DURATION);
        ...
        try {
            ...
            Log.e(LOG_TAG, "processGeofence failed: " + e.getMessage());
        }
    }
